...
public class SumaClass extends Operaciones {
    ...
    /**
     * Creamos una función "suma()" que tiene como parámetros:
    ...
    public void suma(int cantidad) {
        ...
        Double suma = null; // Declaramos una variable "suma" y la inicializamos en null

        // Creamos un bucle "for()" para recorrer la cantidad ingresada por el usuario
        for (int i = 0; i < cantidad; i++) {
            numero = doubleSumaError("Digita el número " + (i + 1) + ": "); // Dentro de "numero", invocamos a la función "doubleSumaError()" y le pasamos una cadena de caracteres

            if (suma == null) {
                suma = numero; // Si el valor de "suma" el null entonces le asignamos el valor de "numero"
            } else {
                suma = suma + numero; // Si no es null, hacemos una operación de suma en cada iteración
            }
        }

            resultado = "\nEl resultado de la suma es: " + suma; // Dentro de "resultado" guardamos una una cadena junto con el resultado de la suma
    }
