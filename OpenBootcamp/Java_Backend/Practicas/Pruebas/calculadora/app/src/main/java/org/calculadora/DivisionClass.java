...
/**
 * Clase que se encargará de ejecutar una operación de división
 * Dentro de esta clase tendremos:
 * Un atributo llamado "resultado"
 * Un constructor para esta clase
 * Una función para calcular la división
 * Una función para evaluar los datos que se ingresen por consola
 * Un método "toString()" que devuelve el resultado de la división
...
public class DivisionClass extends Operaciones {
    ...
    /**
     * Creamos una función "division()" que tiene como parámetros:
    ...
    public void division(int cantidad) {
        ...
        Double division = null; // Declaramos una variable "division" y la inicializamos en null

        // Creamos un bucle "for()" para recorrer la cantidad ingresada por el usuario
        for (int i = 0; i < cantidad; i++) {
            numero = doubleDivisionError("Digita el número " + (i + 1) + ": "); // Dentro de "numero", invocamos a la función "doubleDivisionError()" y le pasamos una cadena de caracteres
            
            if (division == null) {
                division = numero; // Si el valor de "division" el null entonces le asignamos el valor de "numero"
            } else {
                // Si no, evaluamos que no halla un cero dentro de los valores ingresados después del primer valor
                if (numero == 0) throw new ArithmeticException(); // Si existe un cero, lanzamos una excepción de división por cero
                division = division / numero; // Si el valor de "division" es diferente a null, hacemos una operación de división en cada iteración
            }
        }

        resultado = "\nEl resultado de la división es: " + ((double)Math.round(division * 100) / 100); // Dentro de "resultado" guardamos una una cadena junto con el resultado de la división redondeado a 2 decimales
    }
     ...
     */
    public static Double doubleDivisionError(String message) {
